...
//                context.addMessage("msg", new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "An error occurred"));
      ...
      public List<Notification> getFineasta() {
         ...
         public void setFineasta(String username) {
              List<Notification> fineasta = em.createNamedQuery(Notification.findByCategory).setParameter("receiver", username).setParameter("notiftype", Notification.Notiftype.ENDAUCTION).getResultList();
      ...
      public List<Notification> getCambiopos() {
       ...
       public void setCambiopos(String username) {
              List<Notification> cambioposizione = em.createNamedQuery(Notification.findByCategory).setParameter("receiver", username).setParameter("notiftype", Notification.Notiftype.CHANGEPOSITION).getResultList();
  ...
  /*    public void setFineasta( List<Notification> fineasta) {
      ...
      public List<Notification> getTuttelenotifiche() {
          List<Notification> tuttelenotifiche = em.createNamedQuery(Notification.findByReceiver).setParameter("receiver", um.getLoggedUser().getUsername()).getResultList(); // modifico query in modo che mi torni solo quelle dell'utente
